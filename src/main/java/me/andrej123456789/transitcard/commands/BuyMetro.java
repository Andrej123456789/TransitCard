...
public class BuyMetro implements CommandExecutor, TabExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        getEconomy().withdrawPlayer((OfflinePlayer) sender, plugin.getConfig().getDouble("metro_card_price") * hours);

        ItemStack minecart = new ItemStack(Material.MINECART);
        ItemMeta minecart_meta = minecart.getItemMeta();

        minecart_meta.setDisplayName("Metro Minecart (" + Integer.toString(hours) + "h)");
        ...
        if (!plugin.getConfig().getBoolean("electronic_card"))
        {
            ...
            paper_card_meta.setDisplayName("Metro card (" + Integer.toString(hours) + "h)");
            paper_card.setItemMeta(paper_card_meta);

            ((Player) sender).getInventory().addItem(paper_card);
        }

        sender.sendMessage(ChatColor.GREEN + "You have received your metro card for " + Integer.toString(hours) + "h" + ChatColor.RESET);
        return true;
    }
